...
func TestRedirectsNeverReachAnotherRequest(t *testing.T) {
	...
	for _, status := range []int{301, 302, 303, 307, 308} {
		t.Run(fmt.Sprint(status), func(t *testing.T) {
			...
			if err := client.Connect(server.URL, "test-user", "synthetic-password"); err != nil {
				t.Fatal(err)
			}
			defer client.Close()
			roots := x509.NewCertPool()
			roots.AddCert(server.Certificate())
			client.client.Transport.(*http.Transport).TLSClientConfig.RootCAs = roots
			if _, err := client.Execute(http.MethodGet, "data"); err == nil {
