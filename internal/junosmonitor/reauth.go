...
// reauthConnectFunc is a trimmed connectFunc (SSH session only — no NETCONF
// redial) used to restart a device's SSH session after a TACACS
// command-authorization failure. See ConnectDeviceForReauth, the adapter
// callers use to satisfy this from ConnectDevice.
type reauthConnectFunc func(reader *bufio.Reader, host, deviceType string, cache *monitorsetup.CredentialCache) (sessionExecutor, error)

// ConnectDeviceForReauth adapts ConnectDevice to reauthConnectFunc for
// cmd/junos-routing-monitor and cmd/routing-monitor to pass directly to
// NewReauthCoordinator. netconfSnapshot is always false: a TACACS
// command-authorization failure on the polled SSH session says nothing
// about a separate, already-open NETCONF connection (used only for
// before/after snapshots, unaffected by per-command CLI authorization — see
// DeviceSession's doc comment), so reconnecting never redials it.
func ConnectDeviceForReauth(reader *bufio.Reader, host, deviceType string, cache *monitorsetup.CredentialCache) (sessionExecutor, error) {
	client, _, err := ConnectDevice(reader, host, deviceType, false, cache)
	return client, err
}
...
// can pass the same one to both this coordinator and internal/xrmonitor's:
...
// prompt from a Junos device and one from an XR device could interleave on
// the same terminal.
type ReauthCoordinator struct {
	sem        chan struct{}
	reader     *bufio.Reader
	cache      *monitorsetup.CredentialCache
	connect    reauthConnectFunc
	deviceType string
}

// NewReauthCoordinator builds a coordinator that reconnects via connect
// (normally ConnectDeviceForReauth), reusing the same reader/cache/
// deviceType every device was onboarded with. sem must be a channel with
// capacity 1 (typically make(chan struct{}, 1)), shared with any other
// ReauthCoordinator (of this or another platform's package) that prompts on
// the same terminal.
func NewReauthCoordinator(sem chan struct{}, reader *bufio.Reader, cache *monitorsetup.CredentialCache, connect reauthConnectFunc, deviceType string) *ReauthCoordinator {
...
// the same account-lockout reason ConnectDevice itself never retries).
//
