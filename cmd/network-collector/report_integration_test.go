...
func TestLoadConfigDecodesCompactReportTemplate(t *testing.T) {
	path := filepath.Join(t.TempDir(), "workbook.yaml")
	content := `name_playbook: Compact report
report:
  enabled: true
  format: html
  template: compact
`
	...
	if !config.Report.Enabled || config.Report.Template != "compact" {
		t.Fatalf("unexpected compact report configuration: %+v", config.Report)
	}
}
...
func TestReporterGeneratesForEveryWorkflowOperationExample(t *testing.T) {
	...
	for _, path := range files {
		...
		t.Run(parent+"/"+filepath.Base(path), func(t *testing.T) {
			...
			for _, templateName := range []string{"professional", "compact"} {
				reportPath, err := reporting.Generate(reporting.Config{
					RunDir: runDir, SummaryFile: summaryPath,
					Output: templateName + "-report.html", Template: templateName,
				})
				if err != nil {
					t.Fatalf("%s report generation failed for %s: %v", templateName, path, err)
				}
				...
				if !strings.Contains(string(content), "Generated by Network Collector Reporter") {
					t.Fatalf("%s report output for %s is missing the reporter identity", templateName, path)
				}
			}
		})
	}
