...
func TestWorkflowOperationExamplesLoad(t *testing.T) {
	...
	sshDesiredState := loaded["37-declarative-ssh-path.yaml"].SSH[0].Steps
	...
	eosDesiredState := loaded["38-arista-eos-declarative-interface.yaml"].SSH[0].Steps
	if len(eosDesiredState) != 2 || eosDesiredState[0].Approval == nil ||
		eosDesiredState[1].Ensure == nil || eosDesiredState[1].Ensure.Resource != "interface" ||
	...
	iosxeDesiredState := loaded["40-cisco-iosxe-declarative-interface.yaml"].SSH[0].Steps
	if len(iosxeDesiredState) != 2 || iosxeDesiredState[0].Approval == nil ||
		iosxeDesiredState[1].Ensure == nil || iosxeDesiredState[1].Ensure.Resource != "interface" ||
		iosxeDesiredState[1].Ensure.Transport != "ssh" || iosxeDesiredState[1].Ensure.RequireState != "disabled" ||
		!iosxeDesiredState[1].Ensure.RollbackOnFailure {
		t.Fatalf("declarative IOS-XE interface example is incomplete: %+v", iosxeDesiredState)
	}
	iosxeRouteState := loaded["41-cisco-iosxe-declarative-static-route.yaml"].SSH[0].Steps
	if len(iosxeRouteState) != 2 || iosxeRouteState[0].Approval == nil ||
		iosxeRouteState[1].Ensure == nil || iosxeRouteState[1].Ensure.Resource != "static_route" ||
		iosxeRouteState[1].Ensure.Transport != "ssh" || iosxeRouteState[1].Ensure.Prefix == "" ||
		iosxeRouteState[1].Ensure.NextHop == "" || !iosxeRouteState[1].Ensure.RollbackOnFailure {
		t.Fatalf("declarative IOS-XE static-route example is incomplete: %+v", iosxeRouteState)
	}
	nxosDesiredState := loaded["42-cisco-nxos-declarative-interface.yaml"].SSH[0].Steps
	if len(nxosDesiredState) != 2 || nxosDesiredState[0].Approval == nil ||
		nxosDesiredState[1].Ensure == nil || nxosDesiredState[1].Ensure.Resource != "interface" ||
		nxosDesiredState[1].Ensure.Transport != "ssh" ||
	...
	junosDesiredState := loaded["44-junos-declarative-interface.yaml"].SSH[0].Steps
	if len(junosDesiredState) != 2 || junosDesiredState[0].Approval == nil ||
		junosDesiredState[1].Ensure == nil || junosDesiredState[1].Ensure.Resource != "interface" ||
		junosDesiredState[1].Ensure.Transport != "ssh" ||
		junosDesiredState[1].Ensure.RequireState != "disabled" ||
		!junosDesiredState[1].Ensure.RollbackOnFailure {
		t.Fatalf("declarative Junos interface example is incomplete: %+v", junosDesiredState)
	}
	junosRouteState := loaded["45-junos-declarative-static-route.yaml"].SSH[0].Steps
	if len(junosRouteState) != 2 || junosRouteState[0].Approval == nil ||
		junosRouteState[1].Ensure == nil || junosRouteState[1].Ensure.Resource != "static_route" ||
		junosRouteState[1].Ensure.Transport != "ssh" || junosRouteState[1].Ensure.Prefix == "" ||
		junosRouteState[1].Ensure.NextHop == "" || !junosRouteState[1].Ensure.RollbackOnFailure {
		t.Fatalf("declarative Junos static-route example is incomplete: %+v", junosRouteState)
	}
	srosDesiredState := loaded["46-nokia-sros-declarative-port.yaml"].SSH[0].Steps
	if len(srosDesiredState) != 2 || srosDesiredState[0].Approval == nil ||
		srosDesiredState[1].Ensure == nil || srosDesiredState[1].Ensure.Resource != "interface" ||
		srosDesiredState[1].Ensure.Transport != "ssh" ||
	...
	eosVRFState := loaded["49-arista-eos-declarative-vrf.yaml"].SSH[0].Steps
	if len(eosVRFState) != 2 || eosVRFState[0].Approval == nil ||
		eosVRFState[1].Ensure == nil || eosVRFState[1].Ensure.Resource != "vrf" ||
		eosVRFState[1].Ensure.Transport != "ssh" ||
		eosVRFState[1].Ensure.Attributes.RouteDistinguisher == "" ||
		len(eosVRFState[1].Ensure.Attributes.ImportRouteTargets) != 1 ||
