...
// same commands, parsed via xr_bgp_route_table/xr_bgp_advertised_routes so
...
type snapshotResult struct {
	...
	VRFTables map[string]json.RawMessage  `json:"vrf_tables,omitempty"`
...
// captureSnapshot records the full VRF BGP table and, per configured
// neighbor, the routes learned from and advertised to that neighbor. It
// writes two files: a raw <base>.txt (verbatim command output, for a quick
// eyeball diff) and a structured <base>.json (parsed via
// xr_bgp_route_table/xr_bgp_advertised_routes, for a programmatic
...
func captureSnapshot(session *deviceSession, label, outputDir, runLabel string, capturedAt time.Time, parsers map[string]parserModule, out io.Writer) error {
	...
	if len(session.vrfs) > 0 {
		result.VRFTables = map[string]json.RawMessage{}
	}
	for _, vrf := range session.vrfs {
		cmd := fmt.Sprintf("show bgp vrf %s", vrf)
		output, err := session.client.Execute(cmd)
		rawSections = append(rawSections, formatSnapshotSection(cmd, output, err))
		if err != nil {
			result.Errors = append(result.Errors, fmt.Sprintf("vrf_table %s: execute failed: %v", vrf, err))
		} else {
			result.VRFTables[vrf] = parseOrRaw(output, "xr_bgp_route_table", parsers, &result.Errors, "vrf_table "+vrf)
		}
	}
	...
	for _, neighbor := range session.neighbors {
		...
		routesCmd := fmt.Sprintf("show bgp vpnv4 unicast neighbors %s routes", neighbor)
		...
		if err != nil {
			...
			neighborResult.Routes = parseOrRaw(routesOutput, "xr_bgp_route_table", parsers, &result.Errors, "neighbor "+neighbor+" routes")
		}

		advertisedCmd := fmt.Sprintf("show bgp vpnv4 unicast neighbors %s advertised-routes", neighbor)
		...
		if err != nil {
			...
			neighborResult.AdvertisedRoutes = parseOrRaw(advertisedOutput, "xr_bgp_advertised_routes", parsers, &result.Errors, "neighbor "+neighbor+" advertised-routes")
		}
