...
// — a VRF's BGP table, or one neighbor's received/advertised routes — keyed
...
// decodeRouteRecords unpacks one VRF table's or neighbor route list's raw
// JSON (as produced by parseOrRaw with the xr_bgp_route_table/
// xr_bgp_advertised_routes parsers: {"routes": [...]}"). A snapshot whose
...
// different NEXTHOP is reported as changed; ATTRIBUTES/PATH churn (AS-path
// cosmetic differences, MED, etc.) is not, since next hop is what actually
// matters operationally during a change window.
...
// diffSnapshots compares every VRF table and neighbor route/advertised-route
...
func diffSnapshots(before, after snapshotResult) []snapshotDiffSection {
	...
	for vrf := range before.VRFTables {
		vrfNames[vrf] = true
	}
	for vrf := range after.VRFTables {
		vrfNames[vrf] = true
	}
	for vrf := range vrfNames {
		b, bok := before.VRFTables[vrf]
		a, aok := after.VRFTables[vrf]
		sources = append(sources, sectionSource{label: "vrf " + vrf, beforeRaw: b, afterRaw: a, haveBefore: bok, haveAfter: aok})
	}
